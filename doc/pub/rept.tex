...
\usepackage{parskip}
%\usepackage{multicol}  % not used anymore
\usepackage{titlesec}
\usepackage{fancyhdr}
\usepackage{fancyvrb}
...
%\usepackage[table]{xcolor} % not used anymore
...
% fix adobe's PDF reader (it cannot stand xcolor), not used anymore
...
% No PCA was done, it would be great if there was time to do though.

% Pearson coefficient was not used, we do not have any categorical data.
% Instead all data is numeric, correlation coefficient suffices.
...
Only the most important pieces of the code used to perform the experiments are
shown below.  The hard copy of the report contains a CD with the full code used
for the experiments and for the data acquisition.  Appendix
\ref{chap:resources} also explains how to acquire the resources on the CD if
the CD is lost.
